...
function (retPixels) {
var u = JM.Measurement.fixUnits (this.g.measureDistanceUnits.equals ("vdw") ? "angstroms" : this.g.measureDistanceUnits);
var d = this.tm.modelRadius * this.tm.scaleDefaultPixelsPerAngstrom / this.tm.scalePixelsPerAngstrom / 4;
var f = (this.tm.perspectiveDepth ? 1 / this.tm.getPerspectiveFactor ((this.tm.getCameraDepth () - 0.5) * this.getScreenDim ()) : 1);
var m = 0;
var p = 0;
var e = 0;
var min = (this.g.antialiasDisplay ? 30 : 15);
while (p < min) {
e = JM.Measurement.toUnits (d, u, false);
m = Clazz.doubleToInt (Math.floor (Math.log10 (e)));
e = JM.Measurement.fromUnits (Math.pow (10, m) + 0.000001, u);
p = Clazz.floatToInt (e * this.tm.scalePixelsPerAngstrom * f);
if (p < min) {
d *= 10;
}}
var se = (m >= 0 ? " " + Clazz.floatToInt (e) + " " : " 0." + "000000000".substring (0, -1 - m) + "1 ");
retPixels[0] = p;
return se + u;
}, "~A");
Clazz.pu$h(self.c$);
