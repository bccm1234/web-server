...
export default {
  ...
  mounted() {
    let element = jquery("#chargeCanvas");
    let config = { backgroundColor: "#EEF5FF" };
    let viewer = $3Dmol.createViewer(element, config);
    let chargeUri = this.chargeURL;
    jquery.ajax(chargeUri, {
      ...
      error: function (hdr, status, err) {
        console.log(hdr);
        console.log(status);
        console.error("Failed to load cif" + chargeUri + ": " + err);
      }
    });
  },
  methods: {
